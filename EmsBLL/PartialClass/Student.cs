...
namespace EmsBLL
{
    public partial class Student
    {
        ...
        /// <returns></returns>
        public EmsModel.JsonModel GetJsonModel(EmsModel.Student Mod, int pageIndex, int pageSize)
        {
            List<EmsModel.Student> modList = dal.GetListByPageAndSear(Mod, ((pageIndex - 1) * pageSize) + 1, (pageIndex * pageSize));
            //定义分页数据实体
            PagedDataModel<EmsModel.Student> pagedDataModel = null;
            //定义JSON标准格式实体中
            ...
            if (modList.Count > 0)
            {
                ...
                pagedDataModel = new PagedDataModel<EmsModel.Student>()
                {
                    PageCount = pageCount,
                    PagedData = list,
                    PageIndex = pageIndex,
