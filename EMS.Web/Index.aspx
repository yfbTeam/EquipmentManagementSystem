...
<head>
      ...
      window.onload = loading;
        ...
        function BindLeftNavigationMenu() {
            $.ajax({
                ...
                dataType: "jsonp",
                jsonp: "jsoncallback",
                data: { "LoginUID": $("#hid_LoginUID").val(), "action": "GetLeftNavigationMenu" },
                success: function (data) {
                    if (data.result != null) {
                        ...
                        //默认进入视图显示第一个页面
                        var onenav = UrlDate.p || 0, twonav = UrlDate.c || 0;
                        var oneObj = $('#menubox').find('li.onenav:eq(' + onenav + ')');
                        var twoObj = oneObj.children('ul').children('li:eq(' + twonav + ')');
                        oneObj.children('ul').slideDown("fast");
                        //oneObj.click();
                        twoObj.addClass('selected');
                        oneObj.find('.icon-icoxiala').addClass('active');
                        if (onenav != 0 || (onenav == 0 && twonav!=0)) {
                            document.getElementById("iframbox").src = $(twoObj).children('a').attr("data-src");
                        }                        
                    }
                },
 ...
 <body>
    <input type="hidden" id="hid_LoginUID" runat="server" />
	<div class="wrap">
		<!--头部-->
                  ...
                  <div class="rightcontent fl">
                            <div class="login fl" style="position:relative;cursor:pointer;">
                                ...
                                <div class="login_none" style="position:absolute;top:100%;right:10px;width:90px;border-bottom:0;border:1px solid #ccc;background:#fff;z-index:999;display:none;">
                                    <span id="sp_EditPwd" name="sp_EditPwd" runat="server" onclick="javascript: OpenIFrameWindow('修改密码', '/SystemSettings/EditPassword.aspx', '630px', '310px');" style="display:block;width:100%;color:#666;height:30px;border-bottom:1px solid #ccc;cursor:pointer;line-height:30px;text-align:center;">修改密码</span>
                                    <a href="/Index.aspx?action=loginOut" id="loginOut" style="line-height:30px;text-align:center;display:block;width:100%;height:30px;color:#666;">退出</a>
                                </div>
                            </div>
	             </div>     
                   <script> 
